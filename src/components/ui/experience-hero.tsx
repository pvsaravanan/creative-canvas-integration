...
export const ExperienceHero = () => {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen w-full bg-hero flex flex-col selection:bg-foreground selection:text-background overflow-hidden">
      ...
      <div ref={revealRef} className="relative z-10 w-full flex flex-col md:flex-row p-8 md:p-14 lg:p-20 min-h-screen items-center md:items-stretch gap-10">
        <div className="flex-1 min-w-0 flex flex-col justify-between pb-12 md:pb-8 w-full">
          <div className="flex items-center gap-3">
             <div className="relative w-2.5 h-2.5 bg-hero-foreground rounded-full">
                <div className="absolute inset-0 bg-hero-foreground rounded-full animate-ping opacity-30" />
             </div>
             <span className="font-mono text-[11px] font-bold text-hero-foreground tracking-[0.2em] uppercase">HAIK.DEV</span>
          </div>

          <div className="max-w-4xl lg:-translate-y-8 pr-12">
            <h1 className="text-[clamp(3.5rem,9.5vw,11.5rem)] font-black leading-[0.87] tracking-tighter text-hero-foreground uppercase">
              CREATIVE <br /> <span className="text-outline">AGENCY</span>
            </h1>
            <p className="mt-8 font-mono text-[11px] text-hero-muted uppercase tracking-[0.35em] max-w-sm leading-relaxed">
          ...
          <button ref={ctaRef} className="w-fit flex items-center gap-6 group lg:-translate-y-20">
             <div className="w-14 h-14 rounded-full border border-hero-border flex items-center justify-center group-hover:bg-hero-foreground transition-all duration-500 overflow-hidden">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="group-hover:stroke-hero stroke-hero-foreground transition-colors duration-500">
                  <path d="M7 17L17 7M17 7H8M17 7V16" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
             </div>
             <span className="font-mono text-[11px] font-bold text-hero-foreground uppercase tracking-[0.2em]">Start a Project</span>
          </button>
        </div>
        ...
        <div className="w-full md:w-80 lg:w-96 flex-shrink-0 flex flex-col gap-4 justify-center z-20">
          {[
            ...
            <div key={item.id} className="command-cell glass-panel p-6 sm:p-7 block opacity-1">
              <span className="font-mono text-[9px] text-hero-muted uppercase tracking-widest block mb-3">{item.id} // {item.title}</span>
              {item.type === "progress" ? (
                <div className="flex justify-between items-end mt-2">
                  <h4 className="text-2xl sm:text-3xl font-bold text-hero-foreground tracking-tighter">{item.val}</h4>
                  <div className="h-[2px] w-20 bg-hero-border rounded-full overflow-hidden">
                     <div className="h-full bg-hero-foreground w-[60%] animate-loading" />
                  </div>
                </div>
              ) : item.type === "data" ? (
                <div className="mt-4 flex flex-col gap-3">
                  <div className="flex justify-between text-[10px] font-mono text-hero-muted">
                    <span>Awwwards Tier</span>
                    <span>2024-25</span>
                  </div>
                  <div className="h-[1px] w-full bg-hero-border" />
                  <div className="flex justify-between text-[10px] font-mono text-hero-muted">
                ...
                <h3 className="text-sm font-medium text-hero-muted mt-3 leading-snug">
                  Transforming static interfaces into <span className="italic text-hero-foreground">narrative apertures</span>.
                </h3>
              )}
            </div>
          ))}
